...
\section{Home banking -- sample bank: F{\ae}lleskassen}
\label{sec:faellesbanken}

% NB...  fejlstavning i filnavn...
\myimage{gr/faelleskasssen-1}{Web banking in F{\ae}lleskassen}{faelleskassen-1}

\myurl{http://www.w57.dk}{Jan Bertelsen} reports that the Home Banking
facility provided by the Danish bank F{\ae}lleskassen is Java based,
...
Yggdrasil in itself consisted of about 10 pages of perl code, most of
which dealt with generating lists in HTML (most, if not all, of the
list generating code would today be dealt with by generating a tree
structure of the navigation hierachy in XML which would then rendered by
a standard XML-transformation tool).  The search facility was provided
by an \myurl{http://htdig.org}{ht://dig installation}, which rebuilt
its database every night.
