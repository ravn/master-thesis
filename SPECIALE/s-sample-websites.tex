...
in the discussion forum.  See figure~\myvref{fig:slashdot-1} for the top
...
comments submitted by readers (see figure~\myvref{fig:slashdot-2}), and
a login box.
...
figure~\myvref{fig:slashdot-3} is shown.  Note that the layout has
...
The Amazon frontpage is shown in figure~\myvref{fig:amazon-1}.  Note the
...
the corresponding book was returned (see figure~\myvref{fig:amazon-2}).

\myimage{gr/amazon-2}{The result of the search in figure~\myvref{fig:amazon-1}}{amazon-2}
...
\item \textbf{Expected content} - Amazon can be expected to have
  ...
  able to find a given book in the Amazon database, and usually do.  I
  normally find what I am looking for.  John Immerk{\ae}r reports that
  Amazon occasionally do not have all the books he need in their
  database.
...
by users.  At the time of writing, this book has 9
reviews at \textsf{amazon.co.uk}, but 198 at the mother site at
\textsf{amazon.com}.  The corresponding HTML-pages are 22 kb (a good
approximation of the page without comments) and 550
kb respectively.  That is 24 times as many comments as original text.
Not bad.
...
(see figure~\myvref{fig:b-and-o-1}, with three random clickable images
...
as mentioned in
section~\myvref{sec:map-generation-first-web-application}.  In Denmark,
...
shown in figure~\myvref{fig:krak-1}, and is seperated in two parts;
...
map is shown (see figure~\myvref{fig:krak-4}), where it is evident that
...
\myimage{gr/krak-4}{The map in figure~\myvref{fig:krak-3} zoomed out
...
where you must fill in the source yourself.  Figure~\myvref{fig:krak-5a}
...
in figure~\myvref{fig:krak-6}), and is perfectly adequate for a person
...
\myimage{gr/krak-6}{The route in figure~\myvref{fig:krak-5b} explained
  in words}{krak-6}
...
stations, and major bus stops.  Figure~\myvref{fig:dsb-1} shows the initial
...
figure~\myvref{fig:dsb-3}, with departure and arrival times, total
...
figure~\myvref{fig:dsb-4} is shown.  This is a direct connection, and
it is possible to reserve a seat directly from this screen.

A more complicated request is shown in figure~\myvref{fig:dsb-5} where a
...
\item A set of right and left side panes for the navigation.  Clicking
  a link (like those shown in figure~\myvref{fig:yggdrasil-1}) changed
...
This worked very well in the beginning but had a fatal blow when it --
due to the price of Netscape Navigator -- was decided that the company
was to have Microsoft Internet Explorer only.  The users were to edit
their documents in Microsoft Word, which had very poor support for
exporting to HTML and the facilities for uploading documents to the
webserver was also rather tedious to use.  Users of Word could not
...
project, as discussed in chapter~\myvref{cha:cactus}.
