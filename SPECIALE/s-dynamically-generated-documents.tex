...
\myurl{http://www.netcraft.com/Survey/Reports/0002/}{the February 2000
  Netcraft survey} reported that 17,172 NCSA servers were active, and
could be reached from Netcraft.

In August 1995 the number of
\myurl{http://www.netcraft.com/Survey/Reports/9508/ALL/}{webservers
  running the NSCA server was 10,835}, meaning that number has
...
(see \myurl{http://www.freebsd.org}{www.freebsd.org} for details).  As
...
\myvref{fig:xerox-1}) which allowed the user to navigate a
...
% Servlets are discussed in \myvref{sec:CGI-servlets}, PHP3
% scripts in \myvref{sec:CGI-php3}, and Perl (with the mod\_perl
% acceleration module) in \myvref{sec:CGI-modperl}).
...
\myvref{fig:flug-1} show a sample page from the
...
other files with
\myurl{http://www.apache.org/docs/mod/mod_include.html}%
...
\myvref{fig:intel-1} for a sample from
...
that can. \myurl{http://www.google.com}{The Google search engine} was such an engine, and users quickly
...
\myvref{fig:alltheweb-2} and what the fifteenth hit actually
linked to in \myvref{fig:alltheweb-1}. This link was
chosen because the site \textit{expects} the user to have
arrived from a search on these two terms, and even explicitly 
says so in \myvref{fig:alltheweb-1}.
...
``Recently Changed Pages'' (see \myvref{fig:mip-1}) the
...
\myvref{sec:yggdrasil}) was my first attempt to
...
\myurl{http://www.yahoo.com}{Yahoo} (see \myvref{fig:yahoo-1} for the
\myurl{http://www.yahoo.co.uk}{British edition}) which started as a
...
\mycitation{Program documentation is notoriously poor, and maintenance
  ...
  programs are called
  \textit{self-documenting}}{Brooks:  The Mythical Man Month
  \cite{brooks:the-mythical-man-month}}{1975}
...
\myvref{sec:literate-programming}).  The only other publicized works
...
in order to \myurl{http://zone.pspt.fi/pgp/faq/pgp55scan.txt}{scan the
PGP source to circumvent the USA cryptographic export regulations}).
...
question.  See \myvref{fig:acme-1} for a JavaDoc rendered source
...
out either one.  \myvref{fig:sample-pod-file} is the
...
\myvref{fig:pod-sample-1} show the HTML-version as
rendered by a browser.

\begin{figure}[htbp]
  \begin{center}
%\lstinputlisting[language=Perl,frame=trbl]{sample.pod}
\listinginput{1}{sample.pod}
...
\myurl{http://www.fido.dk/faq/unix-faq/unix_r23.htm}%
...
\myurl{http://truetex.com/knuthchk.htm}{actually offers money to those
...
A web-program can be processed in two ways [see \myvref{fig:web-pathways}]:
...
for
\myurl{http://www.rational.com/sitewide/support/whitepapers/dynamic.jtmpl?doc_key=350}{several
...
\myurl{http://www.zdnet.com/zdnn/stories/news/0,4586,2468874,00.html?chkpt=zdhpnews01}{for
  an example from ZDNet}, and \myvref{fig:zdnet-2} for the
...
\myvref{fig:nokia-1}) so such users will probably prefer
...
disabled person.  The
\myurl{http://www.useit.com/alertbox/990613.html}{Jakob Nielsen} and
\myurl{http://www.si.dk/netsteder/publ/tilgaeng/index.html}{Statens
  Information (in Danish)} are good places to start.  Both build upon
the basic work done by
\myurl{http://www.w3.org/TR/WAI-WEBCONTENT/}{the W3C Web Accessory
  Initiative}.  Pages written with these guidelines in mind will work
...
discussed in \myvref{cha:sgml-xml-and-dtd's}.  A
...
% Projects'' -- like \myurl{http://www.linuxdoc.org/}{the Linux
%   Documentation Project} and
% \myurl{http://www.freebsd.org/docproj/docproj.html}{the FreeBSD
...
% \myvref{cha:on-demand-rendering} talks about rendering documents
...
\myvref{cha:cactus}).  \myvref{sec:network-adobe-acrobat-pdf-printer}
...
% Every site but the smallest \textit{must} provide a searching
% facility.  It is essential to let the users locate the information
% they need as fast as possible, in order to leave the impression of a
% well functioning website.  Search results must be fast and accurate!

% Several options exist:

% \begin{itemize}
% \item \textbf{Utilizing a public search engine}  --
% \end{itemize}

% to give the best .  This is essenti
% There a re several approaches to providing search facilities for a web
% site.
