...
On a modern PC Apache has no problem saturating a 10 Mbps Ethernet
connection , meaning that even for very large and demanding sites the
bottleneck should be determined by the hardware!  Phillip Greenspun
talks about databases with added webserver capability in
\cite{phillipandalexsguidetowebpublishing}, and conclude that these are
generally much slower than there non-database relatives.
...
\item When pages are moved or deleted, it becomes difficult to ensure
  that all links pointing to these pages are dealt with
  appropriately.  Jakob Nielsen uses the term \textit{linkrot} to
  describe web sites with many broken links.
...
the bottom of each page is correct.   A script to ensure that all
...
that can.  The Google search engine (see section~\vref{sec:google})
was such an engine, and users quickly started to use it. Recently
links to Amazon.com and Fatbrain.com have begun to crop up in the top
of the search results on Google, and as a direct result users are
going elsewhere.  I have been recommended
\myurl{http://www.alltheweb.com}{AllTheWeb} as a good alternative, but
which after a bit of use appears to be prone to the same polluting
documents as AltaVista.
...
\item \textbf{Size} - which is intended to provide a hint to the user
  about the size of the page in question. 
\end{itemize}
...
development, and the programming language and the programmers working
environment itself helped as much as it possibly could, it would be
easier for the programmers to keep the documentation synchronized with
the code.
...
section~\vref{sec:literate-programming}.
...
or W3C.  \textsf{Some text is missing here}
...
\myimage{gr/acme-1}{The ACME documentation of the \texttt{Acme.Phase}
  class rendered by JavaDoc}{acme-1}
...
Meanwhile, the \myurl{http://xml.apache.org/cocoon/javadoc.html}{an
...
separated in a lot of regions, being either {\TeX}-code or source
code, meaning that the editor cannot provide the supporting functions
which a programmer might have become accustomed to.
...
\myurl{http://w3.pppl.gov/~krommes/fweb.html\#SEC3}{funnelweb}
...
the .tex and .pl files were derived from a web-file.
...
approaches are listed with references.
...
and \myurl{Statens Information [in
  Danish]}{http://www.si.dk/netsteder/publ/tilgaeng/index.html} have
good guidelines for making pages with web content accessible to people
