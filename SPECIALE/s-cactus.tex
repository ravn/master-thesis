...
\myvref{cha:cactus-sample-set-of-filters} list a small set
of external filters I have implemented to provide basic
image and printjob transformations, and
\myvref{cha:cactus-possible-candidates-for-filters} discuss
...
\myimage{gr/cactus-splitted-email-from-lars}{The ``Lidt Guf''-email
  split into components}{cactus-splitted-email-from-lars}

For a user, Cactus is intended to look as shown
in~\myvref{fig:overview}..  As the users use their usual tools to
...
The single most important design decision was to store all data in a
database!  Previously flat-files had been used, but by moving to using
a standard database instead the following benefits were made possible:
...
\myimage{gr/items}{How the examiner update an item, and provide
  information to the extractor.  \textsf{\textit{Note}}: The
  ``Deriver'' box should have been named ``Extractor''.}{items}

Examiners are found in the \texttt{examiners} table (see
\myvref{tab:sql-examiners}.  Figure~\myvref{fig:items}
...
Perhaps the single-most best design desicion was to use a database for
...
\item[Incoming] --
  This table (see~\myvref{tab:sql-incoming}) is used
  by all data acquirers to store their data in as plain strings.  A
  data acquierer should only accept information from known and trusted
  ...
  value too.  The \textsf{src/cactus/process-incoming.pl} script reads
...
I have implemented a very simple search engine which allows users to
look through all entries in the \texttt{items} table, and look for
substrings of the title.

A plain text search.  The user enters a search term, which is
  then used in a \texttt{select ... from items,mime where
    items.mimeid=mime.mimeid and mime.mimetype="text/plain" and
    items.item like "\%\textit{searchstring}\%"}.  This SQL-statement
  will select those text versions which contain the search string
  exactly, and list them.
...
\myimage{gr/cactus-generated-pdf-from-lars}{cactus-generated-pdf-from-lars}{cactus-generated-pdf-from-lars}
\myimage{gr/cactus-splitted-email-from-jimm}{cactus-splitted-email-from-jimm}{cactus-splitted-email-from-jimm}
\myimage{gr/cactus-original-from-lars}{cactus-original-from-lars}{cactus-original-from-lars}
\myimage{gr/cactus-main}{cactus-main}{cactus-main}
\myimage{gr/cactus-documents-pr-user}{cactus-main.pnm cactus-documents-pr-user}{cactus-main.pnm cactus-documents-pr-user}
...
The \filename{\~{}cactus/.procmailrc} contains
