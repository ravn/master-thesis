...
\begin{itemize}
\item Publishing documents must be as easy as possible for
  authors
  
\item The publishing process should be fully automatic,
  without the need for human web masters
  
\item The software requirements of the audience should be as
  small as possible
  
\item The system must run under Linux, and be highly
  customizable
...
\item Allows easy access to the documents through a
  web browser
...
\myurl{http://www.acme.com/software/pbmplus/}{\texttt{pbmplus}
  software package from Jef Poskanzer} which is a very powerful image
manipulation package, based on three very simple image formats (namely
PBM, PGM and PPM for binary, gray-scale and color images
respectively).  Then there is a lot of filters manipulating these
images (these may be piped together to combine effects), and a number
of conversion programs to and from a large set of common graphics
formats.  This approach has proven to be very successful indeed, and
Cactus functions as the arbiter for applying such \textit{filters} to
\textit{items}  in
order to do the conversions and derivations to make the files that the
users require.

Cactus works with items which are considered to be data streams with
attributes owned by somebody.  Cactus itself is only working with the
data as a file containing raw data with a MIME-type, and calls
externally defined filters (may be any kind of program that converts
an input file to an output file) to interpret and manipulate the
information inside as appropriate.

The database tables used by Cactus provide a number of attributes
which may be interpreted and set by the various filters.  This allows
the \textit{presenter} filters to provide better and more informative
views of the items.

By using an underlying database for data storage it is possible to have
simultaneous acquisition, conversion, derivation, compacting and
archiving of data while presenting data as web pages. Experiments have
shown that a modern home PC is sufficient to run Cactus for a small
work group.
...
For a user, Cactus is deceivingly simple.  A casual user follows this algorith:


\begin{enumerate}
\item Submit the document to Cactus by one of many ways, like email or by printing it
...
\item Go to the Cactus home page and locate the upload.  There is a
  ...
  most recent uploads.
\item Find the versions needed of each item.  URLs to items can be
  mailed to others for them to view, and other versions of items can
  be downloaded if so needed.
\end{enumerate}


\myimage{gr/overview}{The grand overview}{overview}


The basic idea is summarized in figure~\vref{fig:overview}.
...
\myimage{gr/gather}{The grand overview}{gather}

boo

\myimage{gr/items}{The grand overview}{items}

baeh

\myimage{gr/xexample}{The grand overview}{xexample}
...
\item[StarOffice 5.1] -- the German office suite for several platforms
  ...
  cannot be automated from the command line.  A StarBasic program must
