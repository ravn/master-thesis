...
\myimage{gr/cactus-splitted-email-from-lars}{The ``Lidt Guf''-email
  from Lars Dalgaard containing a print layout in a scaled and a
  non-scaled PostScript file}{cactus-splitted-email-from-lars}

\myimage{gr/cactus-generated-pdf-from-lars}{The PDF-rendering of Lars'
    print layout}{cactus-generated-pdf-from-lars}
...
\item Go to the Cactus home page and locate the upload.  There is a
  ...
  most recent uploads (see~\myvref{tab:cactus-ten-most-recent} on
...
\item The status page for the item shows all the files that Cactus
  have generated (see \myvref{tab:cactus-splitted-email-from-lars} on
...
tar-archive attached, is shown in \myvref{fig:xexample}.  The
...
The easiest way to do this is as XML (with the additional
...
\item[MIME] -- This table (see \myvref{fig:sql-mime}) contains the mimetypes known to the system.
...
\item[Users] -- This table (see \myvref{fig:sql-users}) contains all users known to the system.  If
...
\item[Workqueue] -- This table (see \myvref{fig:sql-workqueue})
...
An example of why it is necessary to be careful, is the search term
\begin{verbatim}
a\%' ; drop table "items";
\end{verbatim}
...
\myimage{gr/cactus-documents-pr-user}{cactus-documents-pr-user}{cactus-documents-pr-user}
...
\myurl{http://users.ox.ac.uk/~rahtz/passivetex/}{Sebastian Rahtz'
  PassiveTeX} with the \myurl{}{PDF-\TeX} variant of {\TeX} to produce
