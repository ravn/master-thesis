...
\mydetour{ Modern word processors have for a long time been constructed around the
...
\mydetour{ The publisher ``\myurl{http://www.ora.com}{O'Reilly and
...
\myurl{http://www.oreilly.com/people/staff/crism/dsssl/orastyle/}{theirDSSSL
scripts for the purpose available on the net}, allowing others to do
the same.  }
...
tried any of them.  \myvref{tab:distribution-formats} discusses the
...
(see \myvref{sec:docbook-xml-to-rtf} for more details).  The browsers
...
\begin{figure}[htbp]
  \begin{center}
    \textsf{Screenshot}
    \caption{An outline of a DocBook document generated by \textsf{the style sheet} in \myvref{fig:listing-x-outline.xsl}}
...
\myvref{fig:screenshot-x-outline.xsl} for a sample.
...
tag} (named ``/'') is replaced with a \tag{html} and a \tag{body} tag
...
Internet has been a great help.  \myvref{tab:leading-xslt-processors}
...
I started using \myurl{http://xml.apache.org}{Xalan} with Cocoon which
...
that I turned to XT by \myurl{http://www.jclark.com}{James Clark}
